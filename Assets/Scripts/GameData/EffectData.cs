...
namespace MySampleEx
{
    ...
    public class EffectData : BaseData
    {
        ...
        public void LoadData()
        {
            ...
            using (XmlTextReader reder = new XmlTextReader(new StringReader(asset.text)))
            {
                var xs = new XmlSerializer(typeof(Effect));
                effect = (Effect)xs.Deserialize(reder);
        ...
        public EffectClip GetCopy(int index)
        {
            ...
            clip.effentType = original.effentType;
        ...
        //현재 선택한 이펙트 클립 가져오기
        public EffectClip GetCilp(int index)
        {
            //인덱스 체크
            if (index < 0 || index >= this.effect.effectClips.Count)
            {
